...
#echo $tm
tar -cf ${pid}.tar *
coscli cp ${pid}.tar oss://sz-hapdeliver/Kefu_Data_hapdeliver/${tm}/${pid}.tar && coscli signurl sign cos://sz-hapdeliver/Kefu_Data_hapdeliver/${tm}/${pid}.tar  --time 604800 |grep 'https' |awk -F 'https' '{print "https"$NF}' > ${pid}_link.txt

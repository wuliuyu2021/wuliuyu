...
def fastp(outdir, indir, pid, clean_dir, sr1):
	...
	if os.path.exists('/usr/local/bin/fastp'):
		os.system('/usr/local/bin/fastp -i %s -I %s -o %s -O %s  -j %s -h %s -l 150 -n 2 -q 0 -u 40 --disable_adapter_trimming -g --poly_g_min_len 50 -x --poly_x_min_len 10' % (rf1, rf2, gf1, gf2, jf, hf))

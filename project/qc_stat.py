...
def info_writer():
	...
	head_sample_line=('seqID,LaneID,Sample,libID,Raw_Yield(G),Raw_Reads_Num(M),Raw_Q30(%),Raw_Q20(%),'
		'Raw_GC(%),Clean_Yield(G),Clean_Reads_Num(M),Clean_Q30(%),Clean_Q20(%),Clean_GC(%),Effective(%),'
		'Duplication_Rate(%)')
	sam_f=os.path.join(options.outdir, '%s_%s_sample.csv' % 
		('_'.join(os.path.basename(options.seqcsv).split('.csv')[0].split('_')[1:]), options.pid))
	...
	for j in js:
		if '_'.join(j.split('_')[:3]) in ld:
			f=open(j, 'r')
			fastp=Fastp(f)
			sam_lst.write('%s,%s,%s,%s,%.4f,%.2f,%.2f,%.2f,%.2f,%.4f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n' % 
				('_'.join(os.path.basename(options.seqcsv).split('.csv')[0].split('_')[1:]), 
				os.path.basename(j).split('_')[5][3], 
				os.path.basename(j).split('_')[0], 
				os.path.basename(j).split('_')[2], 

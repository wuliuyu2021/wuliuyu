	...
	for tab in tabs:
		...
		with open("%s/%s" %(indir, tab), "r") as f:
			...
			for line in lines:
				...
				AD=lst[70].split(" ")[-1].split(':')[1].split(",")[-1]
				DP=lst[70].split(" ")[-1].split(':')[2]
				
				#if lst[6] in lts and AD!=0 and DP != 0:
				if  int(AD) !=0 and int(DP) != 0 and lst[6] in lts:
					...
					csv1_open.write(
						...
						"\t"+lst[69]+"\t"+lst[70]+"\t"+str(VAF)+"\n")
				if   int(AD) ==0 or  int(DP) == 0 and lst[6] in lts:
					#print("%s--%s"% (AD, DP))

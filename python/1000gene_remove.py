...
def read_table(indir):
	...
	csv_header=(
		...
		'REACTOME_PATHWAY\tGO_BP\tGO_CC\tGO_MF\tKEGG_PATHWAY\tPID_PATHWAY\tBIOCARTA_PATHWAY\tOtherinfo\tAC\tAF\tAN\tDP')
	for tab in tabs:
		csv1=os.path.join(indir, "%s_01.xls" % (os.path.basename(tab).split('.xls')[0]))
		...
		with open("%s/%s" %(indir, tab), "r") as f:
			...
			for line in lines:
				...
				if  lst[50] != "." and float(str(lst[50])) < 0.01 and lst[16] != "." and float(str(lst[16])) < 0.01:
					depth=""
					for x in lst[66].split(" ")[10].split(';'):
						if x.startswith('DP='):
							depth=x.split('DP=')[-1]
					#print(depth)
					csv1_open.write(
						...
						lst[60]+"\t"+lst[61]+"\t"+lst[62]+"\t"+lst[63]+"\t"+lst[64]+"\t"+lst[65]+"\t"+lst[66]+"\t"+
						lst[66].split(" ")[10].split(';')[0].split("AC=")[-1]+"\t"+
						lst[66].split(" ")[10].split(';')[1].split("AF=")[-1]+"\t"+
						lst[66].split(" ")[10].split(';')[2].split("AN=")[-1]+"\t"+
						depth+"\n")
				'''if lst[16] != "." and float(str(lst[16])) < 0.01:
					depth=""
					for x in lst[66].split(" ")[10].split(';'):
						if x.startswith('DP='):
							depth=x.split('DP=')[-1]
					...
					csv1_open.write(
						...
						lst[66].split(" ")[10].split(';')[2].split("AN=")[-1]+"\t"+
						depth+"\n")'''
				if  lst[50] != "." and float(str(lst[50])) < 0.005 and lst[16] != "." and float(str(lst[16])) < 0.005:
					depth=""
					for x in lst[66].split(" ")[10].split(';'):
						if x.startswith('DP='):
							depth=x.split('DP=')[-1]
					...
					csv2_open.write(
						...
						lst[66].split(" ")[10].split(';')[2].split("AN=")[-1]+"\t"+
						depth+"\n")
				'''if lst[16] != "." and float(str(lst[16])) < 0.005:
					depth=""
					for x in lst[66].split(" ")[10].split(';'):
						if x.startswith('DP='):
							depth=x.split('DP=')[-1]

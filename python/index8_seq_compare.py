...
csv1='/thinker/nfs5/public/wuliuyu/index/vazyme.csv'
csv2='/thinker/nfs5/public/wuliuyu/index/agilent.csv'
#csv4='/thinker/nfs5/public/wuliuyu/index/NEB.csv'
csv3='/thinker/nfs5/public/wuliuyu/index/roche-pair.csv'
csv4='/thinker/nfs5/public/wuliuyu/index/IDT.csv'

if os.path.exists('/thinker/nfs5/public/wuliuyu/index/8v-a-r-i.csv'):
	os.remove('/thinker/nfs5/public/wuliuyu/index/8v-a-r-i.csv')
if os.path.exists('/thinker/nfs5/public/wuliuyu/index/8a-r-i.csv'):
	os.remove('/thinker/nfs5/public/wuliuyu/index/8a-r-i.csv')
if os.path.exists('/thinker/nfs5/public/wuliuyu/index/8r-i.csv'):
	os.remove('/thinker/nfs5/public/wuliuyu/index/8r-i.csv')

csv5=open('/thinker/nfs5/public/wuliuyu/index/8v-a-r-i.csv', 'a')
csv6=open('/thinker/nfs5/public/wuliuyu/index/8a-r-i.csv', 'a')
csv7=open('/thinker/nfs5/public/wuliuyu/index/8r-i.csv', 'a')
...
for i in range(len(lst1)):
	if d1.values()[i] in d2.values():
		...
		csv5.write("vazyme,%s,%s,=,agilent,%s,%s\n" % 
		...
		csv5.write("vazyme,%s,%s,=,roche-pair,%s,%s\n" % 
		...
		csv5.write("vazyme,%s,%s,=,IDT,%s,%s\n" % 
			(d1.keys()[i], d1.values()[i], d4i[d1.values()[i]], d4[d4i[d1.values()[i]]]))
	else:
		print("Good luck, roche indexseqs have no duplicates to agilent-roche-pair-IDT!!!")
csv5.close()
...
for j in range(len(lst2)):
	if d2.values()[j] in d3.values():
		...
		d3i = {v:k for k,v in d3.items()}
		csv6.write("agilent,%s,%s,=,roche-pair,%s,%s\n" % 
		...
		d4i = {v:k for k,v in d4.items()}
		csv6.write("agilent,%s,%s,=,IDT,%s,%s\n" % 
			(d2.keys()[j], d2.values()[j], d4i[d2.values()[j]], d4[d4i[d2.values()[j]]]))
	else:
		print("Good luck, vazyme indexseqs have no duplicates to roche-pair-IDT!!!")
csv6.close()
...
for k in range(len(lst3)):
	if d3.values()[k] in d4.values():
		...
		d4i = {v:k for k,v in d4.items()}
		csv7.write("roche-pair,%s,%s,=,IDT,%s,%s\n" % 
			(d3.keys()[k], d3.values()[k], d4i[d3.values()[k]], d4[d4i[d3.values()[k]]]))
	else:
		print("Good luck, agilent indexseqs have no duplicates to IDT!!!")
csv7.close()
...
flag=$(coscli ls -r $ossinfo --include .*_R1_001.fastq.gz |grep "${prefix}" |awk -F " " '{print "'$ossprefix'"$1}')
flagwc=$(coscli ls -r $ossinfo  --include .*_R1_001.fastq.gz |grep "${prefix}" |awk -F " " '{print "'$ossprefix'"$1}' |wc -l)
if [ "$flagwc" -eq 1 ];then
#for file in ${flag[@]};
#do
file=$flag
...
flam=$(coscli ls -r $file --include .*_R1_001.fastq.gz |grep "${prefix}" |awk -F " " '{print "'$ossprefix'"$1}')

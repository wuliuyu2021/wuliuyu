...
echo "cp *sorted.rmdup.bam to $outdir: start"
cp -f $indir/*sorted.rmdup.bam $outdir
echo "cp *sorted.rmdup.bam to $outdir: done"

sh /thinker/nfs5/public/wuliuyu/wuliuyu/work_sh/SamtoolsVariant.sh $outdir $gtf > $outdir/run_bcftools.sh
#sh $outdir/run_bcftools.sh
echo "run bcftools variant cmd maker"
echo "sh $outdir/run_bcftools.sh"

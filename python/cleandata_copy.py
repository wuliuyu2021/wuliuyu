...
def main():
	...
	os.chdir(usbdir)
	#os.system("rename _R1_001_ _ *.gz")
	os.system('cat *.fastq.gz.md5 > md5_old')
	md5new=open("%s/md5" % usbdir, 'a')
	with open("md5_old", 'r') as md5old:
		for line in md5old:
			md5line=line.strip().split('_R1_001_')
			lst1=md5line[0]
			lst2=md5line[1]
			md5new.write('%s_%s\n' % (lst1, lst2))
	md5new.close()
	os.system('rm md5_old *.fastq.gz.md5')

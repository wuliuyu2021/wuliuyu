...
def get_dirnames(indir, pID, seqcsv, contact_id):
	...
	pattern = re.compile(r"(^S\d+)(_%s_)(.+)(_R1)(_001.good.fastq.gz)$" % pID)
...
def main():
	...
	for root, dirs, fs in os.walk(usbdir):
		for f in fs:
			if f.endswith("good.fastq.gz") and f.split('_')[2].startswith("HGC"):
				fr=f.split('_')[2]
				ft=dn[fr]
				#f1='_'.join([str(f.split('_')[0:2]), str(dn[f.split('_')[2]]), str(f.split('_')[3:])])
				f1=f.replace('%s' % fr, '%s' % ft)
				os.system('mv %s/%s %s/%s' % (usbdir, f, usbdir, f1))
	os.chdir(usbdir)			
	for root, dirs, fs in os.walk(usbdir):
		for f in fs:
			if f.endswith("good.fastq.gz"):
				os.system('md5sum %s > %s.md5' % (f, f))
	md5=open('md5', 'a')
	os.system('cat *.good.fastq.gz.md5 > md5')
	os.system('rm *.good.fastq.gz.md5')
